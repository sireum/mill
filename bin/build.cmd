...
def downloadMillRelease(): Unit = {
  ...
  if (!millRelease.exists) {
    ...
    millRelease.downloadFrom(s"https://github.com/sireum/rolling/releases/download/mill/mill-release-$millVersion-scala-2.13.12")
    millRelease.chmod("+x")
    println()
  }
}
...
def madeInteractive(millJar: Os.Path, millBat: Os.Path, mill: Os.Path): Unit = {
  ...
  val bashHeader4: String = ops.StringOps(ops.StringOps(ops.StringOps(
    conversions.String.fromU8is(headerStream.dropWhile(linesF(47)).takeWhile(linesF(2)).toISZ)).
  ...
  val batchHeader1: String = ops.StringOps(conversions.String.fromU8is(headerStream.dropWhile(linesF(57)).takeWhile(linesF(5)).toISZ)).trim
...
//  println(batchHeader3))
  ...
  val offset = findLinesIndex(89) + 1
