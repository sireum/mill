...
def madeInteractive(millJar: Os.Path, millBat: Os.Path, mill: Os.Path): Unit = {
  ...
  val bashHeader4: String = ops.StringOps(ops.StringOps(ops.StringOps(
    conversions.String.fromU8is(headerStream.dropWhile(linesF(37)).takeWhile(linesF(2)).toISZ)).
    replaceAllLiterally("mill.main.client.MillClientMain \"$@\"", "mill.MillMain \"$@\"")).replaceAllLiterally(" $mill_jvm_opts", "")).trim
  val bashHeader5: String = "\nexit\n"
  val batchHeader1: String = ops.StringOps(conversions.String.fromU8is(headerStream.dropWhile(linesF(47)).takeWhile(linesF(5)).toISZ)).trim
  ...
  val batchHeader3: String = ops.StringOps(conversions.String.fromU8is(headerStream.dropWhile(linesF(51)).takeWhile(linesF(3)).toISZ)).trim
  val batchHeader4: String = ops.StringOps(ops.StringOps(ops.StringOps(
    conversions.String.fromU8is(headerStream.dropWhile(linesF(70)).takeWhile(linesF(2)).toISZ)).
    replaceAllLiterally("mill.main.client.MillClientMain %*", "mill.MillMain %*")).replaceAllLiterally(" !mill_jvm_opts!", "")).trim
